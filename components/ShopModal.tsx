...
export const ShopModal: React.FC<ShopModalProps> = ({ onClose, onUpgrade, equipmentLevels, resources, isTestingMode }) => {
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-20" onClick={onClose}>
      ...
      >
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-3xl font-bold text-yellow-300">Equipment Shop</h2>
                    ...
                    <div key={slot} className="bg-stone-900/80 p-4 rounded-lg flex flex-col justify-between">
                        <div>
                            <div className="flex items-center gap-3 mb-2 text-yellow-200">
                                {slotIcons[slot]}
                                <h3 className="text-xl font-bold capitalize">{slot.toLowerCase()}</h3>
                            </div>

                            <div className="flex justify-around items-center my-2 bg-stone-800 p-2 rounded-md">
                                <div>
                                    <p className="text-center text-xs text-stone-400 mb-1">Current</p>
                                ...
                                {nextItem && (
                                    <>
                                        ...
                                        <div>
                                            <p className="text-center text-xs text-green-300 mb-1">Upgrade</p>
                                            <HeroPreviewCanvas 
                            ...
                            <div className="bg-stone-800 p-2 rounded-md mb-2 min-h-[50px]">
                                <p className="font-semibold">{currentItem?.name || 'Unequipped'}</p>
                                <p className="text-sm text-stone-400">
                                    {currentItem ? Object.entries(currentItem.stats).map(([key, val]) => formatStat(key, val!)).join(' / ') : 'No stats'}
                                </p>
                            </div>

                            {nextItem && (
                                <div className="border-t border-stone-700 pt-2">
                                    <p className="text-green-400 font-semibold">{nextItem.name}</p>
                            ...
                            {!nextItem && <p className="text-center text-yellow-400 font-bold mt-4">Max Level!</p>}
                        </div>
